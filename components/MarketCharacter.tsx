...
// ── Holiday prop helpers — all coords are exact canvas pixels ──────────
// hx/hy  = top of right hand pixel   (charX+75, effectiveOY+(21+rightArmDY)*PX)
// lhx/lhy= top of left  hand pixel   (charX,    effectiveOY+(21+leftArmDY)*PX)
// headTopY = effectiveOY (row-0 of character after body/bounce offset)

// Champagne flute — grip at (hx, hy), glass extends UPWARD
function drawChampagneFlute(ctx: CanvasRenderingContext2D, hx: number, hy: number) {
  const x = hx + 2, y = hy
  ctx.fillStyle = '#c8e6fa'; ctx.fillRect(x + 3, y + 18, 3, 14)   // stem
  ctx.fillStyle = '#a0c8e8'; ctx.fillRect(x,     y + 32, 10, 3)   // base
  ctx.fillStyle = '#d6eefa'
  ctx.fillRect(x + 3, y,      5, 4)
  ctx.fillRect(x + 2, y + 4,  7, 4)
  ctx.fillRect(x + 1, y + 8,  9, 4)
  ctx.fillRect(x + 1, y + 12, 9, 4)
  ctx.fillRect(x + 2, y + 16, 7, 2)
  ctx.fillStyle = 'rgba(255,215,0,.5)'; ctx.fillRect(x + 2, y + 6, 7, 10)  // champagne
  ctx.fillStyle = 'rgba(255,255,255,.9)'
  ctx.fillRect(x + 4, y + 8,  2, 2)
  ctx.fillRect(x + 7, y + 12, 2, 2)
}

// Peace sign — center at (cx, cy) — left hand level
function drawPeaceSign(ctx: CanvasRenderingContext2D, cx: number, cy: number) {
  const r = 18
...
function drawCandle(ctx: CanvasRenderingContext2D, cx: number, cy: number, now: number) {
  ctx.fillStyle = '#f8f4e8'; ctx.fillRect(cx - 6, cy, 12, 28)
  ctx.fillStyle = '#ece6d0'; ctx.fillRect(cx - 6, cy + 2, 3, 8); ctx.fillRect(cx + 3, cy + 4, 3, 6)
  ctx.fillStyle = '#444'; ctx.fillRect(cx - 1, cy - 6, 2, 8)
  const flk = Math.sin(now * 0.008) * 2, flk2 = Math.sin(now * 0.013 + 1) * 1.5
  ctx.fillStyle = 'rgba(255,160,0,.25)'; ctx.beginPath()
  ctx.ellipse(cx, cy - 14 + flk, 11, 17, 0, 0, Math.PI * 2); ctx.fill()
  ctx.fillStyle = '#FF8C00'; ctx.beginPath()
  ctx.moveTo(cx - 5, cy - 6)
  ctx.quadraticCurveTo(cx + flk2, cy - 22 + flk, cx, cy - 26)
  ctx.quadraticCurveTo(cx - flk2, cy - 22 + flk, cx + 5, cy - 6)
  ctx.closePath(); ctx.fill()
  ctx.fillStyle = '#FFD700'; ctx.beginPath()
  ctx.moveTo(cx - 3, cy - 6)
  ctx.quadraticCurveTo(cx + flk2 * 0.5, cy - 16 + flk, cx, cy - 20)
  ctx.quadraticCurveTo(cx - flk2 * 0.5, cy - 16 + flk, cx + 3, cy - 6)
  ctx.closePath(); ctx.fill()
  ctx.fillStyle = '#fffde0'; ctx.beginPath(); ctx.ellipse(cx, cy - 10, 2, 3, 0, 0, Math.PI * 2); ctx.fill()
}

// Small flag — pole bottom at (hx, hy) right hand, flag extends up
function drawSmallFlag(ctx: CanvasRenderingContext2D, hx: number, hy: number, now: number) {
  const wave = Math.round(Math.sin(now * 0.004) * 3)
  ctx.fillStyle = '#999'; ctx.fillRect(hx + 4, hy - 52, 3, 58)  // pole
  const fy = hy - 50
  ctx.fillStyle = '#B22234'; ctx.fillRect(hx + 7, fy,       26, 6)
  ctx.fillStyle = '#FFFFFF'; ctx.fillRect(hx + 7, fy + 6,   26, 6)
  ctx.fillStyle = '#B22234'; ctx.fillRect(hx + 7, fy + 12,  26, 6)
  ctx.fillStyle = '#3C3B6E'; ctx.fillRect(hx + 7, fy,       12, 12)
  ctx.fillStyle = '#FFFFFF'
  for (let i = 0; i < 3; i++) for (let j = 0; j < 2; j++)
    ctx.fillRect(hx + 9 + i * 4, fy + 2 + j * 4, 2, 2)
  ctx.fillStyle = 'rgba(255,255,255,.15)'; ctx.fillRect(hx + 24, fy + wave, 8, 18)
}

// Juneteenth confetti burst — orbiting pan-African colored squares
function drawJuneteenthBurst(ctx: CanvasRenderingContext2D, cx: number, cy: number, now: number) {
  const colors = ['#cc0000','#009900','#000000','#ffd700','#cc0000','#009900','#ffd700','#cc0000']
  for (let i = 0; i < 8; i++) {
    const angle = now * 0.003 + i * Math.PI / 4
    const dist  = 28 + Math.sin(now * 0.005 + i) * 8
    const size  = 6 + (i % 3) * 2
...
// Sparkler — stick bottom at (hx, hy) right hand, sparks at top
function drawSparkler(ctx: CanvasRenderingContext2D, hx: number, hy: number, now: number) {
  ctx.fillStyle = '#999'; ctx.fillRect(hx + 3, hy - 60, 4, 64)  // stick
  const tipX = hx + 5, tipY = hy - 62
  ...
  for (let i = 0; i < 12; i++) {
    ...
    ctx.fillRect(tipX + Math.cos(angle) * dist - size/2, tipY + Math.sin(angle) * dist - size/2, size, size)
  }
...
function drawTongs(ctx: CanvasRenderingContext2D, hx: number, hy: number) {
  const tx = hx + 2, ty = hy
  ctx.strokeStyle = '#bbb'; ctx.lineWidth = 3
  ctx.beginPath(); ctx.moveTo(tx,     ty); ctx.lineTo(tx + 8,  ty + 44); ctx.stroke()
  ctx.beginPath(); ctx.moveTo(tx + 9, ty); ctx.lineTo(tx + 16, ty + 44); ctx.stroke()
  ctx.fillStyle = '#888'; ctx.fillRect(tx + 2, ty + 14, 12, 4)
  ctx.fillRect(tx + 6, ty + 40, 3, 10); ctx.fillRect(tx + 13, ty + 40, 3, 10)
}

// Turkey leg — bone grip at (hx, hy), meat extends up-right
function drawTurkeyLeg(ctx: CanvasRenderingContext2D, hx: number, hy: number) {
  const tx = hx + 2, ty = hy
  ctx.fillStyle = '#f5f5f0'; ctx.fillRect(tx + 4, ty + 18, 8, 22); ctx.fillRect(tx + 1, ty + 38, 14, 6)
  ctx.fillStyle = '#c8622a'
  ctx.fillRect(tx,     ty,      16, 6)
  ctx.fillRect(tx - 2, ty + 5,  20, 8)
  ctx.fillRect(tx,     ty + 11, 16, 6)
  ctx.fillRect(tx + 2, ty + 15, 12, 6)
  ctx.fillStyle = '#a0491a'; ctx.fillRect(tx, ty + 6, 4, 12); ctx.fillRect(tx + 12, ty + 5, 4, 12)
...
export default function MarketCharacter({ marketState = 'neutral', changePercent = 0, holidayPreview, bottomOffset = 0 }: Props) {
  ...
  const draw = useCallback(()=>{
    ...
    // ── NEW YEAR'S — bouncing + champagne flute ───────────────────
      ...
      // Attach flute to right hand: effectiveOY + (21+rightArmDY)*PX
      const eOY_ny  = Math.round(s.charY) + extraBounce * PX
      drawChampagneFlute(ctx, Math.round(s.charX) + 77, eOY_ny + (21 + pose.rightArmDY) * PX)
    }
    ...
    // ── PRESIDENTS' DAY — standing tall + Lincoln hat ─────────────
      ...
      s.faceFlush = 0
      renderCharacter(ctx, pose, Math.round(s.charX), Math.round(s.charY), s.direction, 0, 0, 0, W, H)
      // Hat on head — effectiveOY accounts for bodyDY
      const eOY_pres = Math.round(s.charY) + pose.bodyDY * PX
      drawTopHat(ctx, Math.round(s.charX), eOY_pres)
    }
    ...
    // ── GOOD FRIDAY — quiet candlelight vigil ────────────────────
      ...
      s.faceFlush = 0
      renderCharacter(ctx, pose, Math.round(s.charX), Math.round(s.charY), s.direction, 0, 0, 0, W, H)
      // Candle at torso center, waist height
      const eOY_gf = Math.round(s.charY) + pose.bodyDY * PX
      drawCandle(ctx, Math.round(s.charX) + 50, eOY_gf + 80, now)
    }
    ...
    // ── MEMORIAL DAY — slow sway + waving flag ────────────────────
      ...
      pose.leftArmDY  = Math.round(sway * -1)
      pose.rightArmDY = Math.round(-6 + sway * -2)  // arm stays up, slight wave
      pose.leftLegDX  = Math.round(sway * -1)
      pose.rightLegDX = Math.round(sway * 1)
      s.faceFlush = 0
      renderCharacter(ctx, pose, Math.round(s.charX), Math.round(s.charY), s.direction, 0, 0, 0, W, H)
      const eOY_mem = Math.round(s.charY) + pose.bodyDY * PX
      drawSmallFlag(ctx, Math.round(s.charX) + 77, eOY_mem + (21 + pose.rightArmDY) * PX, now)
    }
    ...
    // ── JUNETEENTH — jumping + pan-African confetti burst ─────────
      ...
      s.faceFlush = 0
      renderCharacter(ctx, pose, Math.round(s.charX), Math.round(s.charY), s.direction, 0, extraBounce, 0, W, H)
      // Colorful confetti orbiting above his head
      const eOY_jun = Math.round(s.charY) + extraBounce * PX
      drawJuneteenthBurst(ctx, Math.round(s.charX) + 50, eOY_jun - 20, now)
    }
    ...
    // ── INDEPENDENCE DAY — arm raised + sparkler attached ─────────
      ...
      s.faceFlush = 0
      renderCharacter(ctx, pose, Math.round(s.charX), Math.round(s.charY), s.direction, 0, extraBounce, 0, W, H)
      // Sparkler bottom at right hand
      const eOY_j4 = Math.round(s.charY) + (pose.bodyDY + extraBounce) * PX
      drawSparkler(ctx, Math.round(s.charX) + 77, eOY_j4 + (21 + pose.rightArmDY) * PX, now)
    }
    ...
    // ── LABOR DAY — tending BBQ + animated tongs ─────────────────
      ...
      drawTongs(ctx, Math.round(s.charX) + 77, eOY_lb + (21 + pose.rightArmDY) * PX)
    }
    ...
    // ── THANKSGIVING — eating + turkey leg attached to hand ───────
      ...
      pose.leftArmDY  = 2
      pose.bodyDY     = 1
      s.faceFlush = Math.min(.2, s.faceFlush + .001)
      renderCharacter(ctx, pose, Math.round(s.charX), Math.round(s.charY), s.direction, s.faceFlush, extraBounce, 0, W, H)
      // Turkey leg follows right hand
      const eOY_tg = Math.round(s.charY) + (pose.bodyDY + extraBounce) * PX
      drawTurkeyLeg(ctx, Math.round(s.charX) + 77, eOY_tg + (21 + pose.rightArmDY) * PX)
    }
    ...
    // ── CHRISTMAS — dancing + Santa hat on head ───────────────────
      ...
      s.faceFlush = 0
      renderCharacter(ctx, pose, Math.round(s.charX), Math.round(s.charY), s.direction, 0, extraBounce, 0, W, H)
      // Hat sits on head — effectiveOY = charY + extraBounce*PX (bodyDY=0)
      const eOY_xmas = Math.round(s.charY) + extraBounce * PX
      drawSantaHat(ctx, Math.round(s.charX), eOY_xmas)
    }
