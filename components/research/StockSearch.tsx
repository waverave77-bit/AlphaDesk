          ...
          {results.map((r) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {r.exchange?.trim() && <span className="text-sm text-gray-500">{r.exchange}</span>}
                {r.type?.trim() && <span className="text-sm bg-gray-700 text-gray-300 px-2 py-0.5 rounded">{r.type}</span>}
              </div>
            </button>
          ))}
        </div>
