...
// One-time manual OAuth handoff: Threads redirects here with a short-lived
...
// for a long-lived access token. Nothing sensitive is stored here.
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return page('Threads authorization failed', `<p>Error: <code>${error}</code></p>`)
  }
  if (!code) {
    return page('No authorization code received', `<p>Try the authorization link again.</p>`)
  }

  return page('Threads authorized', `
